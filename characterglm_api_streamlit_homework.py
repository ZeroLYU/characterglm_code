...
运行方式：
```bash
streamlit run characterglm_api_streamlit_homework.py
```
"""
import os
import itertools
...
with st.container():
    ...
    options = ["二次元风格","写实风格","抽象风格","卡通风格","油画风格","水彩风格","像素艺术风格","浮世绘风格","哥特风格","蒸汽朋克风格","极简主义风格","超现实主义风格",
               "印象派风格","表现主义风格","立体主义风格","未来主义风格","波普艺术风格","装饰艺术风格","涂鸦风格"]
    # 设置下拉框的默认值
    default_value = "二次元风格"
    # 显示下拉框，并设置默认选中的选项
    selected_option = st.selectbox("艺术风格", options, key='style_option', on_change=lambda : st.session_state["meta"].update(style_option=st.session_state["style_option"]))
    # 如果用户没有选择任何选项，则将默认值设置为选中
    if selected_option == options[0]:  # 假设选项1是默认值
        selected_option = default_value

def verify_meta() -> bool:
    # 检查`角色名`和`角色人设`是否空，若为空，则弹出提醒
    if st.session_state["meta"]["bot_name"] == "" or st.session_state["meta"]["bot_info"] == ""\
            or st.session_state["meta"]["style_option"] == "":
        st.error("角色名和角色人设和艺术风格不能为空")
        return False
    else:
        return True
...
def draw_new_image():
    ...
    # TODO: 加上风格选项
    style = st.session_state["meta"]["style_option"]
    print(st.session_state["meta"])
    image_prompt = f'{style}。' + image_prompt.strip()
